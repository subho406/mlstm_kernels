...
def mlstm_siging_chunkwise_fw(
    ...
    ),  # last_states (matC_states (B, NH, DHQK, DHHV), vecN_states (B, NH, DHQK))
    None
    | (
        tuple[torch.Tensor, torch.Tensor]
    ),  # all_states (matC_states (B, NH, (NC+1) * DHQK, DHHV), vecN_states (B, NH, (NC+1) * DHQK))
]:
    ...
    matC_k_states, vecN_k_states = mlstm_siging_chunkwise__recurrent_fw_C(
        matK=matK,
        matV=matV,
        vecF=vecF,
        vecI=vecI,
    ...
    if return_all_states:
        ret_tuple += ((matC_k_states, vecN_k_states),)
    else:
        ret_tuple += (None,)
