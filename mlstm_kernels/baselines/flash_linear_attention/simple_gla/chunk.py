...
def chunk_simple_gla_fwd_kernel_h(
    ...
    for i_t in range(NT):
        ...
        b_h += tl.dot(
            b_k,
            (b_v * tl.math.exp2(b_g_last - b_g)[:, None]).to(b_k.dtype),
            allow_tf32=False,
        )
...
def chunk_simple_gla_bwd_kernel_dh(
    ...
    for i_t in range(NT - 1, -1, -1):
        ...
        b_q = (
            b_q
            * scale
            * tl.math.exp2(tl.load(g + i_bh * T + i_t * BT + tl.arange(0, BT)))[None, :]
        ).to(b_q.dtype)
        # [BT, V]
...
def chunk_simple_gla_bwd_kernel_dqkv(
    ...
    for i_v in range(tl.cdiv(V, BV)):
        ...
        # [BT, BV]
        b_dv = tl.dot(b_k, b_dh, allow_tf32=False) * tl.math.exp2(-b_g + b_g_last)[
            :, None
