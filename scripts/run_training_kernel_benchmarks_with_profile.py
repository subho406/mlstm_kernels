...
from mlstm_kernels.benchmark_utils.run_benchmark import run_and_record_benchmarks, run_benchmarks
...
run_training_benchmarks = partial(run_benchmarks, benchmark_creator=create_training_kernel_benchmark)
...
"""
    cfg = from_dict(data_class=BenchmarkConfig, data=OmegaConf.to_container(OmegaConf.create(cfg_yaml)))
...
def run_multiple_benchmarks(output_dir: str = "./outputs_kernel_benchmarks"):
    ...
    with profile(activities=activities, record_shapes=True, profile_memory=True, use_cuda=True,
                 on_trace_ready=torch.profiler.tensorboard_trace_handler(output_folder/"tensorboard")) as prof:
