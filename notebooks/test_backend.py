...
from mlstm_kernels.mlstm.chunkwise import mlstm_chunkwise_triton_stable as mlstm_fwbw_chunkstab
...
if __name__ == "__main__":
    B, H, T, K, V = 1, 1, 2048, 64, 128
    device = "cuda"
    dtype = torch.float32

    q = 1+0.*torch.randn([B, H, T, K], device=device, dtype=dtype)
    k = 1+0.*torch.randn([B, H, T, K], device=device, dtype=dtype)
    ...
    m_i = 0.*torch.ones([B, H], device=device, dtype=dtype)

    mask = torch.randn([B, H, T, V], device=device, dtype=dtype)
    # mask[:, :, ] = 0.
    # mask[:, :, 0] = 0.

    baseline = lambda *x: layer_norm(mlstm_fwbw_chunk(*x, initial_n=n_i, initial_m=m_i, chunk_size=16), ndim=(V,))
    triton = lambda *x: layer_norm(mlstm_fwbw_chunkstab(*x, initial_n=n_i, initial_m=m_i, chunk_size=16), ndim=(V,))
