...
from mlstm_kernels.mlstm.parallel._parallel_stabilized_simple_legacy import (
...
class TestParallelStableTorchVsParallelTorchLong:
    ...
    def test_torch_parallel_unstable_vs_parallel_stabilized_simple(
        self, test_session_folder, S, B, NH, DHQK, DHHV, target_dtype
    ):
        print(f"S{S}B{B}NH{NH}DHQK{DHQK}DHHV{DHHV}")
        template_test_parallel_interface(
            baseline_fn=mlstm_parallel_torch_autograd,
            target_fn=parallel_stabilized_simple,
            baseline_name="unstable_ag",
            target_name="parallel_stabilized_simple_ag",
            S=S,
            B=B,
            NH=NH,
            DHQK=DHQK,
    ...
    def test_torch_parallel_stable_vs_parallel_stabilized_simple(
        self, test_session_folder, S, B, NH, DHQK, DHHV, target_dtype
    ):
        print(f"S{S}B{B}NH{NH}DHQK{DHQK}DHHV{DHHV}")
        template_test_parallel_interface(
