...
from mlstm_kernels.mlstm.parallel import mlstm_parallel_stable_torch_autograd
from mlstm_kernels.mlstm.recurrent import mlstm_recurrent_sequence_torch_step_triton_fused
...
class TestRecurrentSequenceTorchVsParallelTorchAutograd:
    ...
    def test_torch_parallel_stable_vs_unstable_fp32(self, test_session_folder, S, B, NH, DHQK, DHHV):
        ...
        template_test_parallel_interface(
            ...
            target_fn=mlstm_recurrent_sequence_torch_step_triton_fused,
            baseline_name="torch_parallel_stable_ag",
            target_name="torch_recurrent_sequence_step_triton_fused",
            S=S,
            B=B,
            NH=NH,
            DHQK=DHQK,
    ...
    @pytest.mark.skipif(not torch.cuda.is_available(), reason="No GPU available.")
    @pytest.mark.parametrize(["S", "B", "NH", "DHQK", "DHHV"], final_combinations)
    def test_torch_parallel_stable_vs_unstable_bf16(self, test_session_folder, S, B, NH, DHQK, DHHV):
        print(f"S{S}B{B}NH{NH}DHQK{DHQK}DHHV{DHHV}")
        template_test_parallel_interface(
            baseline_fn=mlstm_parallel_stable_torch_autograd,
            target_fn=mlstm_recurrent_sequence_torch_step_triton_fused,
