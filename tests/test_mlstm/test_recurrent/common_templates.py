...
def template_torch_parallel_vs_torch_recurrent_sequence(
    ...
    vecI_p_torch_ag = vecI.clone().to(dtype=baseline_dtype).detach().requires_grad_(True)
    vecF_p_torch_ag = vecF.clone().to(dtype=baseline_dtype).detach().requires_grad_(True)
...
def template_triton_recurrent_sequence_vs_torch_parallel_and_torch_recurrent_sequence(
    S: int = 2048,
    B: int = 2,
    NH: int = 3,
    DHQK: int = 128,  # dim per head
